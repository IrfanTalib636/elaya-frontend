...
export default function StudioNotificationBell() {
  ...
  const {
    ...
    openNotification,
    markAllRead,
  } = useStudioNotifications({ enabled: true })
              ...
              items.map((n) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 border-0 border-b border-elaya-border cursor-pointer transition-colors ${
                    n.unread ? 'bg-studio-gold/5 hover:bg-studio-gold/10' : 'bg-transparent hover:bg-studio-bg-4'
                  }`}
                >
                  <p className="m-0 text-[12px] font-semibold text-studio-white truncate">{n.title}</p>
                  <p className="m-0 mt-0.5 text-[11px] text-studio-w3 line-clamp-2">{n.body}</p>
                </button>
              ))
            )}
          </div>
